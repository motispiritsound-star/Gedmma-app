# Textos de la ficha — español

Para App Store Connect (España) y Google Play Console (es-ES).

## App Store

**Nombre (máx. 30)**
`Bladi — aprender dariya`

**Subtítulo (máx. 30)**
`Árabe marroquí para niños`

**Palabras clave (máx. 100)**
`dariya,darija,marroquí,árabe,marruecos,idioma,niños,aprender,vocabulario,gratis`

**Texto promocional (máx. 170)**
`Nuevo: la aplicación habla español, neerlandés, francés, alemán e inglés, y lee cada palabra con la voz de tu propio dispositivo.`

**Descripción (máx. 4000)**

```
Bladi enseña dariya a niños y adolescentes: el árabe marroquí que la gente habla en casa y en la calle en Marruecos. No el árabe estándar de los manuales, sino la lengua en la que se llama a jeddti.

LECCIONES CORTAS QUE SE QUEDAN
Una lección dura de dos a cuatro minutos. Aprendes a oír una palabra, reconocerla, leerla en escritura árabe, escribirla y decirla en voz alta. Cada palabra vuelve justo antes de que la olvides.

QUÉ APRENDES
• 304 palabras y frases, de salam hasta regatear en el zoco
• 16 unidades que suben de principiante (A0) hasta A2
• Todo el alfabeto árabe, con la forma de cada letra al principio, en medio y al final
• Cuatro conversaciones para leer, con preguntas después
• Tres juegos: carrera contrarreloj, memoria y juego de letras

TODO SUENA
Cada palabra y cada frase se puede escuchar: un toque para la velocidad normal, dos para la lenta. Si tu dispositivo no tiene voz árabe, la aplicación lo dice con honestidad y lee la escritura latina lo mejor que puede.

EN TU IDIOMA
Español, neerlandés, francés, alemán e inglés. La aplicación propone el idioma de tu dispositivo y puedes cambiarlo cuando quieras: traducciones, explicaciones e historias cambian con él.

EMPEZAR GRATIS
Las cinco primeras unidades son gratis: suficiente para presentarte, describir a tu familia y contar hasta cien. Para el curso completo hay una compra única, sin suscripción y sin volver a pagar nunca.

HECHA PARA DÁRSELA A UN NIÑO
• Sin cuenta, sin correo, sin inicio de sesión
• Sin publicidad y sin suscripción
• Sin chat, sin compartir, sin subir nada
• Todo el progreso se queda en el dispositivo
• Los corazones se pueden desactivar: equivocarse forma parte de aprender
• Funciona sin internet, en el avión y en el propio Marruecos

PARA FAMILIAS Y DOCENTES
La aplicación incluye una página que explica cómo se aprende aquí, qué hace y qué no hace, y cómo ayudar en casa. En clase, Bladi funciona en cualquier dispositivo sin instalación ni cuenta.

CON HONESTIDAD SOBRE LA LENGUA
El dariya cambia de ciudad a ciudad y de familia a familia, y no tiene ortografía oficial. Usamos la forma que más se oye en Casablanca y Rabat. ¿Tu abuela lo dice de otra manera? Entonces ella tiene razón, y la propia aplicación lo dice.
```

**Novedades (versión 1.0)**
`La primera versión: 16 unidades, 304 palabras, el alfabeto árabe, historias y juegos. En español, neerlandés, francés, alemán e inglés.`

## Google Play

**Título (máx. 30)**
`Bladi — aprender dariya`

**Descripción breve (máx. 80)**
`Árabe marroquí para niños: lecciones cortas, pronunciación real, empieza gratis.`

**Descripción completa (máx. 4000)**
El mismo texto que la descripción de la App Store.
