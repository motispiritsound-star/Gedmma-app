...
const SECTIONS_NL: Section[] = [
  ['Wat we verzamelen', [
    'Niets. Bladi heeft geen account, vraagt geen e-mailadres en stuurt geen gegevens naar een server.',
  ...
  ['Betalen', [
    'De eerste units zijn gratis. Koop je de volledige cursus, dan gaat die betaling helemaal via de App Store of Google Play: zij innen het bedrag, zij rekenen de btw af en zij weten wie je bent. Bladi krijgt van hen alleen te horen dát er betaald is — geen kaartgegevens, geen adres, geen naam.',
...
const SECTIONS_FR: Section[] = [
  ['Ce que nous collectons', [
    'Rien. Bladi n’a pas de compte, ne demande pas d’adresse e-mail et n’envoie aucune donnée à un serveur.',
  ...
  ['Le paiement', [
    'Les premières unités sont gratuites. Si tu achètes le cours complet, ce paiement passe entièrement par l’App Store ou Google Play : ce sont eux qui encaissent, qui reversent la TVA et qui savent qui tu es. Bladi apprend seulement qu’un paiement a eu lieu — aucune donnée de carte, aucune adresse, aucun nom.',
...
const SECTIONS_DE: Section[] = [
  ['Was wir erheben', [
    'Nichts. Bladi hat kein Konto, fragt keine E-Mail-Adresse ab und schickt keine Daten an einen Server.',
  ...
  ['Bezahlen', [
    'Die ersten Einheiten sind kostenlos. Kaufst du den kompletten Kurs, läuft diese Zahlung vollständig über den App Store oder Google Play: sie ziehen den Betrag ein, sie führen die Mehrwertsteuer ab und sie wissen, wer du bist. Bladi erfährt nur, dass bezahlt wurde — keine Kartendaten, keine Adresse, kein Name.',
...
const SECTIONS_EN: Section[] = [
  ['What we collect', [
    'Nothing. Bladi has no account, asks for no email address and sends no data to a server.',
  ...
  ['Sound', [
    ...
    'The speaking exercises are the one exception: they use your browser’s speech recognition. In Chrome and some others that means the recording goes to the browser’s maker (for Chrome: Google) to be turned into text. Bladi only sees the text and does not keep it. If you would rather avoid that, switch speaking exercises off in the settings; the rest of the app carries on working.',
  ]],
  ['Children', [
  ...
  ['Paying', [
    'The first units are free. If you buy the full course, that payment runs entirely through the App Store or Google Play: they take the money, they account for the VAT and they know who you are. Bladi is only told that a payment happened — no card details, no address, no name.',
...
const SECTIONS_ES: Section[] = [
  ['Qué recogemos', [
    'Nada. Bladi no tiene cuenta, no pide una dirección de correo y no envía datos a ningún servidor.',
  ...
  ['El sonido', [
    ...
    'Los ejercicios de pronunciación son la única excepción: usan el reconocimiento de voz de tu navegador. En Chrome y en algún otro eso significa que la grabación va al fabricante del navegador (en el caso de Chrome, Google) para convertirla en texto. Bladi solo ve el texto y no lo guarda. Si prefieres evitarlo, desactiva los ejercicios de pronunciación en los ajustes; el resto de la aplicación sigue funcionando.',
  ]],
  ['Los niños', [
  ...
  ['El pago', [
    'Las primeras unidades son gratis. Si compras el curso completo, ese pago pasa entero por la App Store o Google Play: ellos cobran el importe, ellos liquidan el IVA y ellos saben quién eres. Bladi solo se entera de que ha habido un pago: ni datos de la tarjeta, ni dirección, ni nombre.',
    'La compra es única; no hay ninguna suscripción en marcha. Antes de poder comprar algo hay una pequeña operación que debe resolver una persona adulta.',
  ]],
  ['Borrar tus datos', [
...
export const PRIVACY: Record<Lang, PrivacyText> = {
  nl: build(
    ...
    'Bladi is gemaakt om aan een kind te kunnen geven. Dit is in het kort wat dat betekent.',
  ...
  fr: build(
    ...
    'Bladi est faite pour pouvoir être confiée à un enfant. Voici ce que cela veut dire, en bref.',
  ...
  de: build(
    ...
    'Bladi ist so gebaut, dass man sie einem Kind in die Hand geben kann. Das heißt kurz gesagt Folgendes.',
  ...
  es: build(
    ...
    'Bladi está hecha para poder dársela a un niño. Esto es, en corto, lo que eso significa.',
  ...
  en: build(
    ...
    'Bladi is built to be handed to a child. Here is what that means, briefly.',
