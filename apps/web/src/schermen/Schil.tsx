      ...
      {ik?.gebruiker?.impersonatie && (
        <div style={{ padding: 'var(--ruimte-3) var(--ruimte-4)' }}>
          <Melding soort="let-op" titel="Ondersteuning kijkt mee">
            Een medewerker van de ondersteuning werkt op dit moment in jouw administratie. Alles wat er
            gebeurt wordt vastgelegd en de toegang stopt automatisch.
          </Melding>
        </div>
      )}
      ...
      <div className="romp">
        <nav className="menu" aria-label={t('nav.hoofdmenu')}>
          <ul className="menu__lijst">
            {MENU.map((item) => (
              <li key={item.pad}>
                <NavLink to={item.pad} end={item.pad === '/'} className="menu__link" aria-current={undefined}>
                  <span aria-hidden="true">{item.teken}</span>
                  <span className="menu__tekst">{t(item.sleutel)}</span>
                  <span className="menu__tekst menu__tekst--kort">{t(item.kort)}</span>
                </NavLink>
              </li>
            ))}
          </ul>
