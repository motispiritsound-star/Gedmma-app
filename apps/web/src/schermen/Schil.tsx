 ...
 * op een groot scherm, en een korte naam voor de tabbalk op een telefoon. Daar
 * staan zeven knoppen naast elkaar en past "Klanten en leveranciers" niet meer
 * leesbaar. Beide staan in de opmaak; het scherm bepaalt met CSS welke er is,
 * zodat een schermlezer er altijd precies een voorleest.
 */
const MENU: {
  pad: string;
  sleutel:
  ...
  kort:
    ...
    | "nav.kort.relaties"
    | "nav.kort.rapporten"
    | "nav.kort.instellingen";
  teken: string;
}[] = [
  {
    pad: "/",
    sleutel: "nav.dashboard",
    kort: "nav.kort.dashboard",
    teken: "◧",
  },
  {
    pad: "/facturen",
    sleutel: "nav.verkoop",
    kort: "nav.kort.verkoop",
    teken: "↗",
  },
  {
    pad: "/inkoop",
    sleutel: "nav.inkoop",
    kort: "nav.kort.inkoop",
    teken: "↙",
  },
  { pad: "/bank", sleutel: "nav.bank", kort: "nav.kort.bank", teken: "≡" },
  {
    pad: "/relaties",
    sleutel: "nav.relaties",
    kort: "nav.kort.relaties",
    teken: "☺",
  },
  {
    pad: "/cijfers",
    sleutel: "nav.rapporten",
    kort: "nav.kort.rapporten",
    teken: "▦",
  },
  {
    pad: "/instellingen",
    sleutel: "nav.instellingen",
    kort: "nav.kort.instellingen",
    teken: "⚙",
  },
];
...
export function Schil({ children }: { children: ReactNode }) {
  ...
  const alleAdministraties = (ik?.organisaties ?? []).flatMap((organisatie) =>
    organisatie.administraties.map((admin) => ({
      ...admin,
          ...
          <label>
            ...
            <select
              ...
              onChange={(gebeurtenis) =>
                kiesAdministratie(gebeurtenis.target.value)
              }
            >
        ...
        <div className="kop__rechts">
          <label className="kop__voorkeur">
            ...
            <select
              ...
              value={taal}
              onChange={(gebeurtenis) =>
                zetTaal(gebeurtenis.target.value as Taal)
              }
            >
              {TALEN.map((item) => (
          ...
          <label className="kop__voorkeur">
            ...
            <select
              ...
              value={thema}
              onChange={(gebeurtenis) =>
                zetThema(gebeurtenis.target.value as Thema)
              }
            >
      ...
      {ik?.gebruiker?.impersonatie && (
        <div style={{ padding: "var(--ruimte-3) var(--ruimte-4)" }}>
          <Melding soort="let-op" titel="Ondersteuning kijkt mee">
            Een medewerker van de ondersteuning werkt op dit moment in jouw
            administratie. Alles wat er gebeurt wordt vastgelegd en de toegang
            stopt automatisch.
          </Melding>
        </div>
      )}
      ...
      <div className="romp">
            ...
            {MENU.map((item) => (
              <li key={item.pad}>
                ...
                >
                  ...
                  <span className="menu__tekst menu__tekst--kort">
                    {t(item.kort)}
                  </span>
                </NavLink>
              </li>
        ...
        <main className="inhoud" id="inhoud" tabIndex={-1}>
          {administratie?.administratie.status === "alleen_lezen" && (
            <Melding
              soort="let-op"
              titel="Deze administratie staat op alleen lezen"
            >
