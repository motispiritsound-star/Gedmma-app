...
describe("openapi", () => {
  test("elke route heeft een omschrijving en elke omschrijving een route", () => {
    ...
    const zonderOmschrijving = eindpunten
      .map(sleutelVan)
      .filter((sleutel) => !omschrijvingen[sleutel]);
    assert.deepEqual(
      zonderOmschrijving,
      [],
      "deze routes staan niet in de specificatie",
    );

    const sleutels = new Set(eindpunten.map(sleutelVan));
    const zonderRoute = Object.keys(omschrijvingen).filter(
      (sleutel) => !sleutels.has(sleutel),
    );
    assert.deepEqual(
      zonderRoute,
      [],
      "deze omschrijvingen wijzen naar routes die niet bestaan",
    );
  });
  ...
  test("de specificatie is zonder aanmelding op te vragen en beschrijft alle paden", async () => {
    ...
    for (const eindpunt of eindpunten) {
      ...
      assert.ok(
        ...
        `methode ontbreekt: ${sleutelVan(eindpunt)}`,
      );
    }
  });
  ...
  test("routes achter een recht dragen dat recht in de specificatie", () => {
    const doc = bouwOpenApi(koppelingen, "0.0.0-test") as {
      paths: Record<
        string,
        Record<string, { "x-vereist-recht"?: string; security?: unknown[] }>
      >;
    };

    const boeking =
      doc.paths[
        "/api/v1/administraties/{administratieId}/verkoopfacturen/{id}/definitief"
      ]?.post;
    ...
    assert.equal(
      boeking.security,
      undefined,
      "deze route hoort niet openbaar te zijn",
    );
  ...
  test("elk pad met een parameter beschrijft die parameter", () => {
    ...
    for (const [pad, inhoud] of Object.entries(doc.paths)) {
      const verwacht = [...pad.matchAll(/\{([A-Za-z0-9_]+)\}/g)].map(
        (treffer) => treffer[1],
      );
      assert.deepEqual(
