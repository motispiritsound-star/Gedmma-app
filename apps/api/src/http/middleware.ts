...
export function requestId(verzoek: Verzoek, antwoord: Response, volgende: NextFunction): void {
  ...
  verzoek.requestId = meegegeven && /^[\w-]{1,64}$/.test(meegegeven) ? meegegeven : randomUUID();
  antwoord.setHeader('X-Request-Id', verzoek.requestId);
  volgende();
}

/** De beveiligingsheaders uit docs/security.md. */
export function beveiligingsheaders(_verzoek: Verzoek, antwoord: Response, volgende: NextFunction): void {
  antwoord.setHeader('X-Content-Type-Options', 'nosniff');
  antwoord.setHeader('Referrer-Policy', 'strict-origin-when-cross-origin');
  antwoord.setHeader('Permissions-Policy', 'camera=(), microphone=(), geolocation=(), payment=()');
  ...
  if (config.isProductie) {
    antwoord.setHeader('Strict-Transport-Security', 'max-age=63072000; includeSubDomains; preload');
  }
  volgende();
}
 ...
 */
export function cors(verzoek: Verzoek, antwoord: Response, volgende: NextFunction): void {
  const oorsprong = verzoek.header('origin');
  const toegestaan = new Set([config.webUrl, 'http://localhost:5173', 'http://127.0.0.1:5173']);
  if (oorsprong && toegestaan.has(oorsprong)) {
    ...
    antwoord.setHeader('Access-Control-Allow-Headers', 'Content-Type, Idempotency-Key, If-Match, X-Request-Id, Accept-Language');
    antwoord.setHeader('Access-Control-Allow-Methods', 'GET, POST, PATCH, PUT, DELETE, OPTIONS');
 ...
 */
export function controleerOorsprong(verzoek: Verzoek, _antwoord: Response, volgende: NextFunction): void {
  ...
  const toegestaan = [config.webUrl, 'http://localhost:5173', 'http://127.0.0.1:5173'];
...
export async function aanmelding(verzoek: Verzoek, _antwoord: Response, volgende: NextFunction): Promise<void> {
  try {
...
export function leesCookie(kop: string | undefined, naam: string): string | undefined {
...
export function eisAanmelding(verzoek: Verzoek, _antwoord: Response, volgende: NextFunction): void {
...
export function vereistRecht(recht: string) {
  return (verzoek: Verzoek, _antwoord: Response, volgende: NextFunction): void => {
...
export function toegangslog(verzoek: Verzoek, antwoord: Response, volgende: NextFunction): void {
  ...
  if (foutje instanceof ApiFout) {
    ...
    if (uitDatabase) {
      ...
      log.error('Onverwachte fout', {
        ...
        stack: foutje instanceof Error ? foutje.stack?.split('\n').slice(0, 6).join('\n') : undefined,
      });
    }
  }

  if (api.status >= 500) {
    log.error('Serverfout', { requestId: verzoek.requestId, code: api.code, melding: api.message });
  }
