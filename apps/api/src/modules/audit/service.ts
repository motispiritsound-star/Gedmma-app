...
export async function controleerKetting(
  client: Db,
  administratieId: string | null,
): Promise<{ id: string; reden: string } | null> {
  const { rows } = await client.query<{
    ...
    `SELECT id::text AS id, op, actor_user_id, actor_soort, actie, onderwerp_soort,
            onderwerp_id, gegevens, organization_id, administration_id, vorige_hash, hash
       FROM audit_event
      WHERE administration_id IS NOT DISTINCT FROM $1
      ORDER BY id ASC`,
    [administratieId],
  ...
  for (const rij of rows) {
    if (rij.vorige_hash !== verwachteVorige) {
      return { id: rij.id, reden: 'De verwijzing naar de vorige regel klopt niet.' };
    }
    const kern = kernVan({
    ...
    if (sha256(`${rij.vorige_hash ?? ''}|${kern}`) !== rij.hash) {
      return { id: rij.id, reden: 'De inhoud van deze regel komt niet overeen met de hash.' };
    }
